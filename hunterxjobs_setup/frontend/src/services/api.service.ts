...
let isOfflineMode = isGitHubPages || (isProduction && !process.env.NEXT_PUBLIC_API_URL);
...
export const getApiBaseUrl = async (forceCheck = false): Promise<string> => {
  // If on GitHub Pages or in offline mode, use mock data and don't try to connect
  if (isGitHubPages || isOfflineMode) {
    console.log('📦 GitHub Pages or offline mode detected - using mock data');
    return '';
  }
...
export const apiRequest = async (
  ...
  // If on GitHub Pages or in offline mode, return mock data
  if (isGitHubPages || isOfflineMode) {
    console.log(`📦 Using mock data for ${endpoint} (GitHub Pages/offline mode)`);
