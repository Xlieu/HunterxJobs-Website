...
exports.initiateLinkedInAuth = async (req, res) => {
  try {
    ...
    if (!config.linkedin || !config.linkedin.clientId || !config.linkedin.clientSecret) {
      logger.error('LinkedIn authentication attempted but LinkedIn is not properly configured');
      return res.status(503).json({
        success: false,
        message: 'LinkedIn integration is not configured. Please contact the administrator.'
      });
    }
    ...
    if (token) {
      try {
        ...
        logger.warn('Failed to get user from token during LinkedIn auth:', err.message);
    ...
    if (req.session) {
      ...
      logger.warn('Session not available for LinkedIn authentication');
    }
